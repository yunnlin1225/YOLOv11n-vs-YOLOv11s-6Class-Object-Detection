...
ROOT = Path(r"C:\Users\user\Desktop\YOLOv11n_6Categories(2)")
...
def create_folds():
    ...
    if len(positive_images) != 900:
        raise ValueError(
            f"\n❌ 正樣本應為 900 張，目前偵測到 {len(positive_images)} 張！"
        )

    if len(background_images) != EXPECTED_BACKGROUND:
        raise ValueError(
            f"\n❌ 背景圖應為 {EXPECTED_BACKGROUND} 張，"
    ...
    print("\n✅ 資料確認正確：900 張正樣本 + 150 張背景 = 1050 張")
    ...
    positive_splits = list(
        positive_kf.split(positive_images)
    )

    background_splits = list(
        background_kf.split(background_images)
    )
    ...
    for fold in range(1, 6):
        ...
        # 刪除舊 Fold，避免沿用以前 900 張版本
        if fold_dir.exists():
            print(f"⚠ Fold {fold} 已存在，刪除後重新建立...")
...
def train_5fold():
    print(
        f"\n🚀 開始 YOLOv11n 6 類別 + "
        f"{EXPECTED_BACKGROUND} 張背景圖五折訓練"
    )
    ...
    for fold in range(1, 6):
        ...
        model.train(
            ...
            project="YOLOv11n_6Class_5Fold_BG150",
            name=f"Fold_{fold}",
            exist_ok=True
        )
