...
# 💡 請確認 ROOT 指向你合併好「6 個類別」總資料集的根目錄
...
def train_5fold():
    print(f"\n🚀 開始 YOLOv11n 6 類別五折訓練（資料夾：{ROOT.name}，每折 {EPOCHS} 輪）")
    ...
    for fold in range(1, 6):
        ...
        model.train(
            ...
            device=0,
            workers=WORKERS,
            cache=False,                   # 關閉快取避免記憶體累積
            amp=True,                      # 開啟混合精度加速
            lr0=0.005,                     # 初始學習率
            cos_lr=True,                   # Cosine 學習率衰減
            close_mosaic=10,               # 最後 10 輪關閉 Mosaic 提升精準度
            project="YOLOv11_6Class_5Fold", # 專案資料夾名稱
            name=f"Fold_{fold}",
            exist_ok=True
        )
