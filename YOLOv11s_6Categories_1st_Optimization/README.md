# YOLOv11s 第一次優化
### YOLOv11s First Optimization with Hyperparameter Tuning

## 📌 專案介紹（Project Introduction）

本專案為 YOLOv11s 六類別物件偵測模型的**第一次優化階段**，以 **Ultralytics YOLOv11s** 物件偵測模型為核心，建立包含熊（Bear）、貓（Cat）、狗（Dog）、人（Person）、猴子（Monkey）及豬（Pig）之六類別物件偵測模型。

本研究延續 YOLOv11n 六類別模型實驗，使用相同資料集與五折交叉驗證（5-Fold Cross Validation）方式進行 YOLOv11s 模型訓練與效能評估。

第一次優化主要針對 **Classification Loss Weight（`cls`）與 Bounding Box Loss Weight（`box`）** 進行多組超參數測試，並依據研究設定之效能目標選擇適合的參數組合。
...
### 模型與訓練設定（Model & Training Settings）
...
### 五折交叉驗證設定（5-Fold Cross Validation Settings）
...
經多組超參數測試後，本研究最終將 **Classification Loss Weight（`cls`）設定為 0.745**，並將 **Bounding Box Loss Weight（`box`）設定為 8.0**，作為 YOLOv11s 五折交叉驗證的最終訓練設定。
...
### 五折資料結構（5-Fold Dataset Structure）
...
### 五折交叉驗證結果（5-Fold Cross Validation Results）
...
整體結果顯示，最終參數設定下的 YOLOv11s 具備良好的六類別目標偵測能力。:contentReference[oaicite:2]{index=2}


## 🔧 第一次優化：超參數調整（First Optimization: Hyperparameter Tuning）

為提升 YOLOv11s 的整體偵測效能，第一次優化主要針對 **Classification Loss Weight（`cls`）與 Bounding Box Loss Weight（`box`）** 進行超參數調整，共測試 **4 組參數組合**，並以 Precision、Recall、mAP50 及 mAP50-95 作為評估依據。

### 超參數測試結果（Hyperparameter Tuning Results）
...
YOLOv11s 在第一次優化後的最終參數設定 **`cls=0.745`、`box=8.0`** 下，五折平均 Precision 為 **90.08%**、Recall 為 **90.18%**、mAP50 為 **93.30%**、mAP50-95 為 **77.58%**，四項指標皆達成本研究設定之效能目標。

### 🎯 研究目標達成情況（Research Objective Achievement）
...
整體而言，YOLOv11s 在不同資料切分下皆維持良好的偵測表現。雖然各 Fold 間存在一定程度的效能差異，但五折平均結果均達成研究目標，顯示第一次優化後的參數設定具有良好的六類別目標偵測能力。

> **綜合五折交叉驗證結果，經第一次超參數優化後的 YOLOv11s 已達成 Precision、Recall、mAP50 及 mAP50-95 四項研究效能目標。**


## 🎚️ 信心度閾值分析（Confidence Threshold Analysis）
...
### 📈 Confidence Threshold 比較（Threshold Comparison）
...
考量本研究最終目標為建立可供實際部署之六類別目標偵測模型，因此除 Precision 外，亦需兼顧目標檢出能力與 Bounding Box 定位表現。

綜合比較結果，本研究最終採用 **Confidence Threshold = 0.25**，作為後續模型推論與最終部署模型之信心度門檻。

> **Confidence Threshold = 0.25 能在 Precision、Recall 與定位效能之間維持較佳的整體平衡，並同時達成本研究設定之四項效能目標，因此選定作為最終部署模型的推論設定。**


## 🖼️ 模型推論結果（Inference Results）

使用第一次優化後所選定之 YOLOv11s 模型，並設定 **Confidence Threshold = 0.25**，針對六類別測試影像進行實際推論，以驗證模型於未參與訓練影像上的目標辨識能力。
...
本研究完成 YOLOv11s 六類別目標偵測模型的**第一次優化**，並透過 **5-Fold Cross Validation** 驗證模型於不同資料切分下的偵測效能與穩定性。

第一次優化主要針對 **Classification Loss Weight（`cls`）與 Bounding Box Loss Weight（`box`）** 進行超參數調整。經多組參數測試後，最終選定 **`cls=0.745`、`box=8.0`** 作為模型訓練設定。
...
> **綜合五折交叉驗證、第一次超參數優化、信心度閾值評估與實際推論結果，本研究完成 YOLOv11s 第一次優化模型之效能驗證，並取得 `cls=0.745`、`box=8.0` 的最終參數設定，作為後續第二次優化與模型實際部署的基礎。**
