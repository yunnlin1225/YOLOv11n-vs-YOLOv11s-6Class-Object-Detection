...
# 1. 基本設定
# ==========================================
# 填入你的最佳模型權重路徑 (可改為你的 5 折最佳權重，例如 Fold 1 的 best.pt)
MODEL_PATH = r'C:\Users\user\Desktop\YOLOv11s_6Categories(1)\runs\detect\YOLOv11_6Class_5Fold\Fold_1\weights\best.pt'

# 最佳實務門檻設定 (P、R、mAP50 🟢達標 > 0.90)
CONF_THRES = 0.25  # 信心度門檻 (Confidence Threshold)
IOU_THRES = 0.60   # 非極大值抑制重疊門檻 (NMS IoU Threshold)
IMGSZ = 640        # 評估與推論影像解析度 (Image Size)

# 測試目標 (可填入圖片路徑、影片路徑或包含多張圖片的資料夾路徑)
# 範例圖片路徑: r'C:\Users\user\Desktop\test_img.jpg'
# 範例影片路徑: r'C:\Users\user\Desktop\test_video.mp4'
INPUT_SOURCE = r'C:\Users\user\Desktop\test_img.jpg'


def run_inference(
    model_path, source_path, conf=0.25, iou=0.60, save_txt=False
):
  ...
  print(
      f'🔍 開始進行推論 (設定 conf={conf}, iou={iou}, 來源:'
      f' {source_path})...'
  )
