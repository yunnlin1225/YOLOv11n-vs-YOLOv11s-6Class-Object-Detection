...
本專案旨在建立一套具備良好偵測準確度與定位能力的六類別目標偵測模型，並透過五折交叉驗證（5-Fold Cross Validation）評估 YOLOv11n 模型在不同資料切分下的穩定性與泛化能力，作為後續模型推論與實際部署之基礎。
...
4. **評估模型實際應用與部署可行性**
   - 以 YOLOv11n 輕量化模型為基礎，在兼顧模型偵測效能與運算效率的情況下，評估其於實際應用環境中的部署可行性。
   - 完成模型效能驗證與 Confidence Threshold 評估後，建立 YOLOv11n 最終模型，作為後續實際部署與應用測試之基礎。
...
| -------------------: | ---------: | ---------: | ---------: | ---------: |
| **0.25（Default）** |     0.9068 |     0.9088 | **0.9458** | **0.7874** |
...
結果顯示，Confidence Threshold = **0.26** 在 Precision 與 Recall 上略有提升，但 **0.25** 在 mAP50 與 mAP50-95 上具有較佳表現。考量後續模型實際推論與部署需兼顧目標檢出能力、辨識準確度與 Bounding Box 定位表現，因此本研究不以單一指標最高值作為門檻選擇依據。

> **綜合四項評估指標，本研究最終採用 Confidence Threshold = 0.25，作為 YOLOv11n 後續模型推論與實際部署之信心度門檻。**
...
> **綜合五折交叉驗證、信心度閾值評估與實際推論結果，本研究完成 YOLOv11n 優化模型之效能驗證。後續將使用完成優化之 YOLOv11n 建立最終模型並進行實際部署，並與 YOLOv11s 的部署結果進行比較，進一步分析兩種模型於實際應用環境下的偵測效能與運行表現。**
