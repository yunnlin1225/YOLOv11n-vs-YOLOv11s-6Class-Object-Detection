...
其中較明顯的問題為 **Class Misclassification（類別誤判）**。影片中的實際目標為 **Bear**，但於部分連續影格中，模型會將 Bear 錯誤辨識為 **Person** 或 **Pig**，使同一目標在影片過程中出現類別短暫切換的現象。

此外，當目標受到遮擋、距離改變或場景較複雜時，亦可能出現 Bounding Box 短暫消失或偵測不連續的情況。

雖然本階段透過提高 Image Size 至 `1024` 改善部分影片中的偵測表現，但仍無法完全消除 Bear 被誤判為 Person 或 Pig 的問題。
